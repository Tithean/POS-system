...
function InvoiceModal({ data, InvoceNumber, InvoiceDate }) {
  const invoiceItems = data || [];
  const totalAmount = invoiceItems.reduce(
    (total, item) => total + (item.TotalAmount || Number(item.Quantity || 0) * Number(item.Price || 0)),
    0
  );
      ...
      <div className="modal-box receipt-content max-w-2xl bg-white text-[#1e3a5f] rounded-3xl p-8 shadow-2xl border border-gray-200">
        ...
        <div className="grid grid-cols-2 gap-8 py-4 border-t border-gray-100 text-sm">
          ...
          <div className="flex flex-col gap-1.5 text-right">
            <div className="flex justify-between text-xs">
              ...
              <span className="font-semibold text-[#1e3a5f]">{formatDate(InvoiceDate)}</span>
            </div>
            <div className="flex justify-between text-xs">
            ...
            <tbody>
              {invoiceItems.length ? (
                invoiceItems.map((item, index) => {
                  const name = item.ProductId?.ProductName || item.prod_name || "Product";
                  const qty = Number(item.Quantity || item.qty || 1);
                  const price = Number(item.Price || item.price || 0);
                  const lineTotal = item.TotalAmount ? Number(item.TotalAmount) : qty * price;
        ...
        <div className="modal-action receipt-actions flex gap-3 mt-6">
          ...
          <form method="dialog" className="flex-1">
            <button className="w-full py-2.5 rounded-xl bg-[#1e3a5f] text-white font-bold text-sm hover:opacity-90 transition-opacity">
              បិទ (Close)
            </button>
          </form>
        </div>
